...
class Hooks {
	...
	public static function callBack( \Parser $parser, $world = 'All', $vocation = '', $amount = '25' ) {
		...
		if (!in_array($world, $worlds)) {
			return '<div class="error">Mundo inexistente.</div>';
		}
		if (empty($vocation)) {
			$vocation = 'any';
		}
		...
		if (!in_array($vocation, $vocations)) {
			return '<div class="error">Vocação inválida.</div>';
		}
		$vocationAddon = '';
		if ($vocation != 'any') {
		...
		if ( !is_numeric( $amount ) || intval( $amount ) <= 0 ) {
			return '<div class="error">Quantidade precisa ser um valor numérico positivo.</div>';
		}
		...
		$content = $cache->getWithSetCallback( $cache->makeKey( 'tibiahighscores', $world, $vocation, $amount ), $cache::TTL_HOUR, function() use ( $world, $vocation, $vocationAddon, $amount ) {
			...
			$table = '<table class="wikitable"><tr><th></th><th>Nome</th><th>Vocação</th><th>Level</th><th>Guilda</th></tr>';
